function KSTM=Conducts(TTM,PATM)
...
KSTM=struct('OL',zeros(NIY+2,NIX+2),...%olivine thermal conductivity [W/m/K]
    'OPX',zeros(NIY+2,NIX+2),...%opx thermal conductivity [W/m/K]
    'CPX',zeros(NIY+2,NIX+2),...%cpx thermal conductivity [W/m/K]
    'PL',zeros(NIY+2,NIX+2),...%pl thermal conductivity [W/m/K]
...
% for i=1:NIX+2
%     for j=1:NIY+2
%         Tintg=a0*(TTM(j,i)-298)+0.5*a1*(TTM(j,i)^2-298^2)+a2*(-1.0/TTM(j,i)+1.0/298.0);%Eq.(10) in Hofmeister 1999
%         
%         KLA=K298*(298.0/TTM(j,i))^a*exp(-Tintg*(1.0/3.0+4.0*ga))*(1.0+dKTdP*PAG(j,i)/KT)^((4.0*ga+1.0/3.0)/dKTdP);%lattice only
%         KRA=0.01753-0.00010365*TTM(j,i)+2.2451e-7*TTM(j,i)^2-3.407e-11*TTM(j,i)^3;%radiation only, for Ferrous minerals, Hofmeister 1999
%         KSTM.OL(j,i)=KLA+KRA;%lattice+radiation
%     end
% end
...
for i=1:NIX+2
    for j=1:NIY+2
        Tintg=a0*(TTM(j,i)-298)+0.5*a1*(TTM(j,i)^2-298^2)+a2*(-1.0/TTM(j,i)+1.0/298.0);%Eq.(10) in Hofmeister 1999
        KLA=K298*(298.0/TTM(j,i))^a*exp(-Tintg*(1.0/3.0+4.0*ga))*(1.0+dKTdP*PAG(j,i)/KT)^((4.0*ga+1.0/3.0)/dKTdP);%lattice only
        KRA=8.5e-11*TTM(j,i)^3;%radiation only, for Fe-free silicates and oxides, Hofmeister 1999
        KSTM.OL(j,i)=KLA+KRA;%lattice+radiation
    end
end
...
%     KLA=K298*(298.0/TTM(j,i))^a*exp(-Tintg*(1.0/3.0+4.0*ga))*(1.0+dKTdP*PAG(j,i)/KT)^((4.0*ga+1.0/3.0)/dKTdP);%lattice only
%     KRA=0.01753-0.00010365*TTM(j,i)+2.2451e-7*TTM(j,i)^2-3.407e-11*TTM(j,i)^3;%radiation only, for Ferrous minerals, Hofmeister 1999
%     KSTM.OPX(j,i)=KLA+KRA;%lattice+radiation
%     end
% end
...
K298=4.5;%Hofmeister1999
ga=0.96;%Hofmeister1999
a=0.33;

for i=1:NIX+2
    for j=1:NIY+2
        Tintg=a0*(TTM(j,i)-298)+0.5*a1*(TTM(j,i)^2-298^2)+a2*(-1.0/TTM(j,i)+1.0/298.0)+0.2*a3*(TTM(j,i)^5-298^5);%Caption of Table 3 in Schutt&Lesher2006
        KLA=K298*(298.0/TTM(j,i))^a*exp(-Tintg*(1.0/3.0+4.0*ga))*(1.0+dKTdP*PAG(j,i)/KT)^((4.0*ga+1.0/3.0)/dKTdP);%lattice only
        KRA=8.5e-11*TTM(j,i)^3;%radiation only, for Fe-free silicates and oxides, Hofmeister 1999
        KSTM.OPX(j,i)=KLA+KRA;
    end
end

%[2]: Hofmeister2012 Table 2
%KLA=1.0e-6/(-0.67915+0.0051741*TTM(j,i)-3.6072e-6*TTM(j,i)^2+1.0339e-9*TTM(j,i)^3);%up to 1275 K
...
%     for j=1:NIY+2
%         
%         if(TTM(j,i)<=1100.0)%max fit for T 1100 K
%             Dlat=1.0e-6/(-0.36581+0.0040009*TTM(j,i)-15.671e-7*TTM(j,i)^2);%[m^2/sec]
%             Hd=155.2+0.006285*TTM(j,i)-923000.0/TTM(j,i)^2-1020.0/sqrt(TTM(j,i));%[J/mol/K]
%             Hd=2.0*Hd/0.2480924;%CaFeSi2O6: 248.0924 g/mol [J/kg/K]
%             RHO=densityfull(TTM(j,i),PAG(j,i));%input K, GPa
%             KLA=Dlat*Hd*RHO{30,2};%30: clino-Hd
%             KRA=0.01753-0.00010365*TTM(j,i)+2.2451e-7*TTM(j,i)^2-3.407e-11*TTM(j,i)^3;%radiation only, for Ferrous minerals like olvine, Hofmeister 1999
%             %Assuming Aegirine NaFe(III)Si2O6 as Hd end-member, its lattice diffusivity up to 1100 K
%             KSTM.CPX(j,i)=KRA+KLA;
%         else
...
for i=1:NIX+2
    for j=1:NIY+2
        if(TTM(j,i)<=1400.0)%Lattice saturation tempearture 1400 K
            D=1.0e-6/(-0.22631+0.0023854*TTM(j,i)-6.9593e-7*TTM(j,i)^2);%thermal diffusivity from Hofmeister 2008 Table 2 [m^2/sec]
            %NOTE: we assume this diopside is pure Di from Hofmerister 2008
            Di=157.3+0.0000205*TTM(j,i)-1372950.0/TTM(j,i)^2-1010.5/sqrt(TTM(j,i));%[J/mol/K]
            Di=2.0*Di/0.2165504;%CaMgSi2O6: 216.5504 g/mol [J/kg/K]
            RHO=densityfull(TTM(j,i),PAG(j,i));%input K, GPa
            KLA=Di*D*RHO{29,2};%29: clino-Di
            KRA=8.5e-11*TTM(j,i)^3;%radiation only, for Fe-free silicates and oxides, Hofmeister 1999.
            KSTM.CPX(j,i)=KLA+KRA;
        else
            Di=157.3+0.0000205*TTM(j,i)-1372950.0/TTM(j,i)^2-1010.5/sqrt(TTM(j,i));%[J/mol/K]
            Di=2.0*Di/0.2165504;%CaMgSi2O6: 216.5504 g/mol [J/kg/K]
            RHO=densityfull(TTM(j,i),PAG(j,i));%input K, GPa
...
% for i=1:NIX+2
%     for j=1:NIY+2
%         if(TTM(j,i)<=1400.0)%Tg=1260 K, melting point 1400 K
%             %Albite [010],[001],[100] mean value as D, data from Table 6 of Hofmeister 2009
%             D=1.0/(0.493+257.26/TTM(j,i)+27922.0/TTM(j,i)^2)+...
%                 1.0/(0.31095+314.91/TTM(j,i))+...
%                 1.0/(0.33964+105.38/TTM(j,i)+30164.0/TTM(j,i)^2);%mm^2/sec
%             D=1.0e-6/(D/3.0);%m^2/sec
%             %Note: bulk value is calculated following Eq.(6) in Branlund 2012
%             
%             %It's Analbite
%             Ab=309.74+0.015278*TTM(j,i)-26160000.0/TTM(j,i)^2+4109100000.0/TTM(j,i)^3+8840.0/TTM(j,i);%[J/mol/K]
...
% for i=1:NIX+2
%     for j=1:NIY+2
%         D_bulk=1.0/(0.4183+82.33/TTM(j,i)+9356.8/TTM(j,i)^2)+...
%             1.0/(0.699+99.88/TTM(j,i)+25726.6/TTM(j,i)^2)+...
%             1.0/(0.5492+110.4/TTM(j,i)+15574.4/TTM(j,i)^2);%mm^2/sec
%         D_bulk=3.0/(D_bulk*1.0e-6);%m^2/sec
%         %Note: bulk value is calculated following Eq.(6) in Branlund 2012
%         
%         D_melt=0.52615-9.1562/TTM(j,i)+9920.3/TTM(j,i)^2;%mm^2/sec
...
for i=1:NIX+2
    for j=1:NIY+2
        D=(0.36+0.4*exp(-(TTM(j,i)-273.0)/300.0))*1.0e-6;%[m^2/sec]
        
        b=b0+2.0*b1*(TTM(j,i)-298.0);
        RHO=densityfull(TTM(j,i),PAG(j,i));%input K, GPa
        An=290.90+0.0276*TTM(j,i)-34080000.0/TTM(j,i)^2+5218000000.0/TTM(j,i)^3+29625.0/TTM(j,i);%[J/mol/K]
...
% for i=1:NIX+2
%     for j=1:NIY+2
%         if(TTM(j,i)<=1360.0)%fitting range
%             D=1.0e-6/(-0.10641+0.0017914*TTM(j,i)-6.4912e-7*TTM(j,i)^2);%[m^2/sec]
%             
%             %Grossular
%             Gs=542.6+0.01294*TTM(j,i)-3186000.0/TTM(j,i)^2+277700000.0/TTM(j,i)^3-56020.0/TTM(j,i);%[J/mol/K]
%             Gs=Gs/0.450446376;%Ca3Al2Si3O12: 450.446376 g/mol [J/kg/K]
%             RHO=densityfull(TTM(j,i),PAG(j,i));%input K, GPa
%             KLA=D*Gs*RHO{37,2};%37: Grossular
%             KRA=8.5e-11*TTM(j,i)^3;%radiation only, for Fe-free silicates and oxides, Hofmeister 1999
%             KSTM.GRT(j,i)=KLA+KRA;
%         end
%     end
% end
% 
...
% for i=1:NIX+2
%     for j=1:NIY+2
%         if(TTM(j,i)<=573.0)%fitting range
%             D=1.0e-6/(-0.26309+0.0024971*TTM(j,i)-1.4377e-6*TTM(j,i)^2);%[m^2/sec]
%             
%             %Andradite set as Gs
%             Gs=542.6+0.01294*TTM(j,i)-3186000.0/TTM(j,i)^2+277700000.0/TTM(j,i)^3-56020.0/TTM(j,i);%[J/mol/K]
%             Gs=Gs/0.450446376;%Ca3Al2Si3O12: 450.446376 g/mol [J/kg/K]
...
%         KLA=K298*(298.0/TTM(j,i))^a*exp(-Tintg*(1.0/3.0+4.0*ga))*(1.0+dKTdP*PAG(j,i)/KT)^((4.0*ga+1.0/3.0)/dKTdP);%lattice only
%         KRA=8.5e-11*TTM(j,i)^3;%radiation only, for Fe-free silicates and oxides, Hofmeister 1999
%         KSTM.GRT(j,i)=KLA+KRA;%lattice+radiation
%     end
% end
...
% for i=1:NIX+2
%     for j=1:NIY+2
%         if(TTM(j,i)<=500.0)%fitting range
%             D=1.0e-6/(-0.56653+0.0046202*TTM(j,i)-3.4391e-6*TTM(j,i)^2);%[m^2/sec]
%             
%             Mag=-3.558e3+3.3473e2*sqrt(TTM(j,i))-9.309*TTM(j,i)+2.5388e-3*TTM(j,i)^2+1.4273e5/TTM(j,i);%[J/mol/K] from Robie 1978
...
%         KLA=K298*(298.0/TTM(j,i))^a*exp(-Tintg*(1.0/3.0+4.0*ga))*(1.0+dKTdP*PAG(j,i)/KT)^((4.0*ga+1.0/3.0)/dKTdP);%lattice only
%         KRA=8.5e-11*TTM(j,i)^3;%radiation only, for Fe-free silicates and oxides, Hofmeister 1999
%         KSTM.SP(j,i)=KLA+KRA;%lattice+radiation
%     end
% end
...
for i=1:NIX+2
    for j=1:NIY+2
        if(TTM(j,i)<=1360.0)
            KSTM.ILM(j,i)= -2.7e-12*TTM(j,i)^4+8.3e-9*TTM(j,i)^3-7.4e-6*TTM(j,i)^2+1.3e-3*TTM(j,i)+2.3;%[W/m/K]
        end
    end
end

%% =====================================================
KSTM.OL=Border(KSTM.OL);
KSTM.OPX=Border(KSTM.OPX);
KSTM.CPX=Border(KSTM.CPX);
KSTM.PL=Border(KSTM.PL);
%KSTM.SP=Border(KSTM.SP);
%KSTM.GRT=Border(KSTM.GRT);
KSTM.ILM=Border(KSTM.ILM);
%NOTE: KS at ghost cells are set equal to inner layer since the
