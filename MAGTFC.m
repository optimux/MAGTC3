...
for i=2:NIX+1
    for j=2:NIY+1
        TN(j,i)=(TQDT(j-1,i-1)+IHS(j,i)+TQVT(j-1,i-1)+LHT(j,i))/TFN(j,i);%[J/m^3 divided by J/m^3/K == K]
    end
end
...
for i=1:NIX
    ...
    for j=1:NIY
        ...
        %0<FSTTM(j+1,i+1)<FSE, TTM(j+1,i+1)==TLTM(j+1,i+1)>TE, TN(j+1,i+1)<TE, TLN(j+1,i+1)<TE
        caseID(j,i)=3;
        ...
        if((CLCM(j+1,i+1)>CE.MgO)&&(TN(j+1,i+1)>=TE)&&(TN(j+1,i+1)<=TLTM(j+1,i+1)))
            ...
            %0.0<=FST<FSE, TE<=TN<=TLTM --> CLCM>CE.MgO, TE<=TN<=TLTM
            caseID(j,i)=2;
        end
        if((CLCM(j+1,i+1)<=CE.MgO)&&(FSTTM(j+1,i+1)+dFSLHCAL<=1.0)&&(TN(j+1,i+1)<TE))
            %CLCM=MCL.MgO: Critical Major element that represents system evolution, e.g., MgO in liquid of Di-An system [1]
            %SITUATION 4: EUTECTIC
            caseID(j,i)=4;
        end
        if((FSTTM(j+1,i+1)<1.0)&&(TN(j+1,i+1)<TE)&&(dFSLHCAL+FSTTM(j+1,i+1)>1.0))
            %SITUATION 5: EUTECTIC --> COMPLETE SOLID
            caseID(j,i)=5;
        end
        if((FSTTM(j+1,i+1)>=1.0)&&(TTM(j+1,i+1)<=TE))
            %SITUATION 6: COMPLETE SOLID COOLING
            caseID(j,i)=6;
        end
        ...
        switch caseID(j,i)
            ...
            case 2 %SITUATION 2: LIQUID --> MUSHY REGION
            ...
            case 5 %SITUATION 5: EUTECTIC --> COMPLETE SOLID
                ...
                FLN(j+1,i+1)=0.0;%here, dFSSMT(j,i)=FLTM(j+1,i+1)-dFSLHT(j+1,i+1)
