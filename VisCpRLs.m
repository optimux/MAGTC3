function [MU,CPL,RL] = VisCpRLs(TTM,PATM,MCLTM,FSTM)
%This function will firstly calculate melt viscosity from emperical prediction of Shaw (1972) and
%then estimate bulk viscosity (melt+crystal) from Switching function; also Cp of
%melt will be calculated; also liquid density. 's' stands for plural, or multiple grids.
%See also single grid calculation counterpart VisCpRL.m.
%Created on 2020-6-17

%TTM: [K] [NIY+2,NIX+2]
%PATM: absolute pressure [Pa] [NIY+2,NIX+2]
%MCLTM: temporary MCL of size [NIY+2,NIX+2]
...
All=zeros(NIY+2,NIX+2);

nSiO2=zeros(NIY+2,NIX+2);
nTiO2=zeros(NIY+2,NIX+2);
nAl2O3=zeros(NIY+2,NIX+2);
nFeO=zeros(NIY+2,NIX+2);
nFe2O3=zeros(NIY+2,NIX+2);
nMnO=zeros(NIY+2,NIX+2);
nMgO=zeros(NIY+2,NIX+2);
nCaO=zeros(NIY+2,NIX+2);
nNa2O=zeros(NIY+2,NIX+2);
nK2O=zeros(NIY+2,NIX+2);
nP2O5=zeros(NIY+2,NIX+2);
nH2O=zeros(NIY+2,NIX+2);
nAll=zeros(NIY+2,NIX+2);

xSiO2=zeros(NIY+2,NIX+2);
xTiO2=zeros(NIY+2,NIX+2);
xAl2O3=zeros(NIY+2,NIX+2);
xFeO=zeros(NIY+2,NIX+2);
xFe2O3=zeros(NIY+2,NIX+2);
xMnO=zeros(NIY+2,NIX+2);
xMgO=zeros(NIY+2,NIX+2);
xCaO=zeros(NIY+2,NIX+2);
xNa2O=zeros(NIY+2,NIX+2);
xK2O=zeros(NIY+2,NIX+2);
xP2O5=zeros(NIY+2,NIX+2);
xH2O=zeros(NIY+2,NIX+2);
xAll=zeros(NIY+2,NIX+2);

SumSlope=zeros(NIY+2,NIX+2);
MeanSlope=zeros(NIY+2,NIX+2);
lnLiqVisco=zeros(NIY+2,NIX+2);
MU=zeros(NIY+2,NIX+2);

CPL=zeros(NIY+2,NIX+2);
RL=zeros(NIY+2,NIX+2);
xRSiO2=zeros(NIY+2,NIX+2);
xRTiO2=zeros(NIY+2,NIX+2);
xRAl2O3=zeros(NIY+2,NIX+2);
xRFeO=zeros(NIY+2,NIX+2);
xRFe2O3=zeros(NIY+2,NIX+2);
xRMnO=zeros(NIY+2,NIX+2);
xRMgO=zeros(NIY+2,NIX+2);
xRCaO=zeros(NIY+2,NIX+2);
xRNa2O=zeros(NIY+2,NIX+2);
xRK2O=zeros(NIY+2,NIX+2);
xRP2O5=zeros(NIY+2,NIX+2);
xRH2O=zeros(NIY+2,NIX+2);
nRAll=zeros(NIY+2,NIX+2);
VL=zeros(NIY+2,NIX+2);
ML=zeros(NIY+2,NIX+2);
...
MU=exp(lnLiqVisco)/10000.0;%[Pa.sec]   /10.0

MU(1,2:NIX+1)=MU(2,2:NIX+1);
MU(NIY+2,2:NIX+1)=MU(NIY+1,2:NIX+1);

MU(1:NIY+2,1)=MU(1:NIY+2,2);
MU(1:NIY+2,NIX+2)=MU(1:NIY+2,NIX+1);
...
RL(1,2:NIX+1)=RL(2,2:NIX+1);
RL(NIY+2,2:NIX+1)=RL(NIY+1,2:NIX+1);

RL(1:NIY+2,1)=RL(1:NIY+2,2);
RL(1:NIY+2,NIX+2)=RL(1:NIY+2,NIX+1);
...
CPL(1,2:NIX+1)=CPL(2,2:NIX+1);
CPL(NIY+2,2:NIX+1)=CPL(NIY+1,2:NIX+1);

CPL(1:NIY+2,1)=CPL(1:NIY+2,2);
CPL(1:NIY+2,NIX+2)=CPL(1:NIY+2,NIX+1);
